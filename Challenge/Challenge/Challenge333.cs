...
namespace Challenge
{
    internal class Challenge333 : Challenge
    {
        ...
        {
            var messages = new Dictionary<int, List<(int packetId, string msg)>>();

            foreach (var packet in _input)
            {
                if (!messages.TryGetValue(packet.messageId, out var msg))
                {
                    msg = new List<(int, string)>();
                    messages.Add(packet.messageId, msg);
                }

                msg.Add((packet.packetId, packet.message));

                if (!Complete()) continue;

                foreach (var msgPacket in msg.OrderBy(p => p.packetId))
                {
                    Console.WriteLine($"{packet.messageId} {msgPacket.packetId} {msgPacket.msg}");
                }

                messages.Remove(packet.messageId);

                bool Complete() => msg.Count == packet.packetCount;
            }
        }

        private readonly List<(int messageId, int packetId, int packetCount, string message)> _input =
            new List<(int, int, int, string)>
            {
                (6220, 1, 10, "Because he's the hero Gotham deserves"),
                (6220, 9, 10, string.Empty),
                (5181, 5, 7, "in time, like tears in rain. Time to die."),
                (6220, 3, 10, "So we'll hunt him."),
                (6220, 5, 10, "Because he's not a hero."),
                (5181, 6, 7, string.Empty),
                ...
                (2997, 18, 19, string.Empty),
                (9949, 9, 10, string.Empty),
